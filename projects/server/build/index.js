...
// projects/server/src/http/index.ts
var import_http2 = __toESM(require("http"));

// projects/server/src/lib/http.ts
function createRTreeNode() {
  return {
    nodes: {}
  };
}
function traverse(tree, pathArr, options = {}) {
  if (pathArr.length === 0) {
    return tree;
  }
  if (tree.nodes[pathArr[0]] === void 0) {
...
function getRequestHandler(tree, method, pathArr) {
...
function addRequestHandler(tree, method, path, fn) {
...
function createHTTPHandler() {
  ...
  async function onRequest(req, res) {
    ...
    const requestHandler = getRequestHandler(rootRTree, method, pathArr);
    ...
    try {
      const parsedReq = Object.assign({}, req, { searchParams });
      const data = await requestHandler(parsedReq);
      const response = Object.assign(createAPIResponse(), { data });
      res.writeHead(200);
      ...
      const response = Object.assign(createAPIResponse(), {
        error: {
          message: err.message,
          code: err.code,
          name: err.name
...
// projects/server/src/http/api.ts
var API_VERSION = 1;
function registerAPIEndpoints(withClient, GET2) {
  GET2("/", () => {
    return {
      version: API_VERSION
    };
  });
  GET2("/android", async (req) => {
...
function startServer(withClient, httpHost2, httpPort2) {
  return new Promise((resolve, reject) => {
    const { onRequest, GET: GET2 } = createHTTPHandler();
    const server = import_http2.default.createServer(onRequest);
    registerAPIEndpoints(withClient, GET2);
    server.listen(httpPort2, httpHost2, () => {
      resolve(true);
    });
  });
}

// projects/server/src/lib/xray_db.ts
